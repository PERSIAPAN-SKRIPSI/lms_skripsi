    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
                {{ __('Add Subcategory to ') }} {{ $category->name }}
            </h2>
        </div>
    </x-slot>

    <div class="py-8">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">  {{-- Ubah max-width menjadi 3xl --}}
                    ...
                    <form action="{{ route('admin.categories.sub-categories.store', $category) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex items-center justify-end mt-4">
                           <a href="{{ route('admin.categories.sub-categories.index', $category) }}"
                               class="inline-flex items-center px-4 py-2 bg-gray-200 border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition ease-in-out duration-150 mr-4">  {{-- Added mr-4 --}}
                                Back
                            </a>
                            <button type="submit"
